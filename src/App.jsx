...
export default function App() {
  ...
  const copyAll = async () => {
    ...
    const body = isPrac
      ? PRACTICE.map((c) => `${c.n} [${c.tag}] ${c.q.replace(/\n/g, " ")}\n→ ${getText(c.id) || "(빈 카드)"}\n`)
  ...
  const doReset = async () => {
    /* 아카이브에서 보고 있는 덱(deckMode)의 기록만 비운다 */
    const u = userRef.current;
    setResetBusy(true);
    const isPrac = deckMode === "practice";
    const ids = new Set((isPrac ? PRACTICE : CARDS).map((c) => c.id));
    const kept = {};
    Object.entries(answersRef.current).forEach(([k, v]) => { if (!ids.has(k)) kept[k] = v; });
    if (u) {
      try {
        const col = await getDocs(answersColRef(u.uid));
        const batch = writeBatch(db);
        const prefix = `${PROGRAM_ID}__`;
        col.forEach((docSnap) => {
          if (!docSnap.id.startsWith(prefix)) return;
          if (ids.has(docSnap.id.slice(prefix.length))) batch.delete(docSnap.ref);
        });
        /* 구버전 answers 맵은 남길 덱의 답만 담아 필드 교체(update)로.
           문서가 없으면 update가 실패하니 먼저 존재를 보장한다. */
        await setDoc(userDocRef(u.uid), { email: u.email || null }, { merge: true });
        batch.update(userDocRef(u.uid), { answers: kept });
        await batch.commit();
      } catch (e) {
        setResetBusy(false);
    ...
    setAnswers(kept);
    try { await store.save(activeKeyRef.current, JSON.stringify(kept)); } catch (e) { /* 무시 */ }
    ids.forEach((id) => { writeQueues.current.delete(id); cloudDirty.current.delete(id); });
    setResetBusy(false);
    setIdx(0); setAskReset(false);
    popToast(isPrac ? "첫,눈 기록을 비웠어요" : "반,짝임 기록을 비웠어요");
  };
      ...
      {askReset && (
        <div className="about-backdrop" onClick={() => setAskReset(false)}>
          <div className="about-card confirm-card" role="alertdialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="confirm-q">{deckMode === "practice" ? "첫,눈" : "반,짝임"}의 답을 모두 지울까요? 되돌릴 수 없어요.</p>
...
/* 밝은(크림) 표면 버튼: 빛줄을 올리브 톤으로 올려 크림 카드 위에서도 또렷하게 */
.ghost::before,.primary.alt::before,.google-btn::before{
  background:linear-gradient(90deg,transparent 0%,rgba(122,132,100,.2) 16%,rgba(122,132,100,.5) 36%,rgba(103,112,84,.72) 50%,rgba(122,132,100,.5) 64%,rgba(122,132,100,.2) 84%,transparent 100%);
  filter:blur(.6px);
  box-shadow:0 0 8px 2px rgba(122,132,100,.32);
}
/* 빛줄 톤 팔레트 정합: 주황 마치기엔 크림빛 */
.btn-finish::before{
...
.arch-card.filled{border-left:3px solid #C69E7D} /* 채도 낮춘 클레이 톤 — 본문 오렌지 텍스트보다 한 톤 가라앉힘 */
