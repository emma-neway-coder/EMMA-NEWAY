...
/* 로그인해야 볼 수 있는 화면 — 보관함만. 덱은 3장까지 로그인 없이 체험 가능 */
const GATED_VIEWS = new Set(["archive"]);
/* 체험 한도: 덱마다 3장까지는 자유롭게, 4장째 입력면부터 로그인 */
const TRIAL_LIMIT = 3;
const sentences = (t) =>
  t.includes("\n")
...
export default function App() {
  ...
  /* ── 체험 게이트: 비로그인은 덱마다 채워진 카드 3장까지.
     4장째(아직 빈 카드)의 입력면부터 잠긴다. 이미 쓴 카드는 계속 고칠 수 있다. */
  const [authNotice, setAuthNotice] = useState(false);
  const trialSheetShown = useRef(false); // 시트는 세션당 자동으로 한 번만
  const trialGated = (cardId) => {
    if (user) return false;
    const list = deckMode === "practice" ? PRACTICE : MAIN_WRITABLE;
    const filled = list.filter((c) => getText(c.id)).length;
    return filled >= TRIAL_LIMIT && !getText(cardId);
  };
              ...
              <div className="flip-inner">
                ...
                <section className="face front" aria-hidden={flipped} {...(flipped ? { inert: "" } : {})}>
                  ...
                  <div className="face-bottom">
                    ...
                    <button className="primary no-seam" onClick={openBack}>
                ...
                <section className="face back" aria-hidden={!flipped} {...(!flipped ? { inert: "" } : {})}>
                  ...
                  <div className="face-bottom">
                    <button className="primary wide no-seam" onClick={closeBack}>덮어두기</button>
                  </div>
                </section>
...
.primary{
  ...
  border:1px solid rgba(87,80,63,.06);border-radius:12px;padding:13px 26px;cursor:pointer;
  background-clip:padding-box;
  transition:transform .13s ease, background .18s ease, box-shadow .13s ease;
  box-shadow:0 1px 0 rgba(255,255,255,.16) inset, 0 1px 1.5px rgba(87,80,63,.12), 0 7px 16px rgba(92,101,74,.16);
  animation:seamDoor .6s cubic-bezier(.22,.61,.36,1) both;
}
@keyframes seamDoor{
  0%{ clip-path:inset(0 50% 0 50% round 12px) }
  100%{ clip-path:inset(0 0 0 0 round 12px) }
}
/* 문틈 빛(등장): 크림빛이 글자와 같은 속도로 좌우로 번지고 스민다 */
.primary::after{
...
/* 카드덱 뒤집기/덮어두기 버튼: 문틈 등장 효과(seamDoor+doorLight)만 제거, 터치 빛줄(::before)은 유지 */
.primary.no-seam{ animation:none; clip-path:none }
.primary.no-seam::after{ content:none }
/* 반짝임(터치=:active): 문장 가로 정가운데 틈에서 옅은 올리브빛이 좌우로, 떼면 사라진다 */
...
.primary:active::before,.ghost:active::before{
...
.ghost{
  position:relative; overflow:hidden;
  font-family:inherit;font-size:13px;color:var(--olive-deep);background:var(--card);
  border:1px solid var(--line);border-radius:12px;padding:12px 22px;cursor:pointer;
  box-shadow:0 1px 0 rgba(255,255,255,.8) inset, 0 3px 9px rgba(87,80,63,.07);
...
@media (prefers-reduced-motion: reduce){
  .primary,.ghost,.arrow,.btn-finish,.why-toggle,.dots button{transition:none}
  .primary:hover,.ghost:hover,.arrow:hover:not(:disabled),.btn-finish:hover{transform:none}
  .primary{animation:none;clip-path:none}
  .primary::after{animation:none;opacity:0}
  .primary::before,.ghost::before{transition:none}
}
...
.arrow:hover:not(:disabled){background:#fff;transform:translateY(-1px)}
...
.btn-finish{transition:transform .12s ease, box-shadow .12s ease, background-color .15s ease;
  box-shadow:0 1px 0 rgba(255,255,255,.25) inset, 0 3px 0 #8F5527, 0 6px 14px rgba(143,85,39,.24)}
.btn-finish:hover{background-color:#B06B39;transform:translateY(-1px)}
...
.primary.alt{background:var(--card);color:var(--olive-deep);border:1px solid var(--olive);
  box-shadow:0 1px 0 rgba(255,255,255,.8) inset, 0 3px 9px rgba(87,80,63,.07)}
